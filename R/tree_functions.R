...
add_variable <- function(tree,
  ...
  } else { # Using the approach where any variable can be added
    index_available_interactions <- 1:(length(data$basis_subindex))
  }
...
change_stump <- function(tree,
  ...
  # Calling the function change_interaction for the stump if any_var is true
  if(data$any_var){
    new_tree <- change_variable(tree = tree,
                                curr_part_res = curr_part_res,
                                data = data)
    return(new_tree)
  }
...
change_variable <- function(tree,
  ...
  # Special case when the length of the tree is one
  if(length(tree)==1){
    c_node <- tree$node0
  } else {
    # Sampling a terminal node
  ...
  # For the MASTER-VARIABLapproach only interactionss should be considerated
  if(isFALSE(data$any_var)){
      # Recalling the vector of the interactions
      all_interactions_index_ <- (length(data$dummy_x$continuousVars)+1):length(data$basis_subindex)
    ...
    # For the case where any variable can be included
    index_available_interactions <- 1:(length(data$basis_subindex))
    replaced_interaction_candidates <- c_node$pred_vars[c_node$pred_vars %in% index_available_interactions]
